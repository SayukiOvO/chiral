...
16. **出站分流与中转线路机制相同、意图不同，所以是两张表**（M6）：中转线路是**给订阅者看的一条线**（有名字、有权限、进订阅）；出站分流是**机器自己的路由**（按 geosite/geoip 决定落点，订阅者看不见也选不了），落点为自有节点时不要求先建中转线路。装配次序是硬性的：**受限拦截 > 出站分流 > 中转规则 > 骨架规则**——中转规则不带目的地条件、匹配该用户的一切连接，排它后面的分流永远不命中（变异测试守着）。两条渲染铁律：geosite 与 geoip 必须**拆成两条规则共用一个 tag**（同规则内条件是 AND，写一起匹配不到任何东西）；**匹配为空的规则一律拒绝**（没有目的地条件 = 匹配全部，会吞掉整台节点）。同一外部节点被中继和被分流当落点时**共用同一条 outbound**。**落点是自有节点时必须继承落点的受限目的地**——和中转入口同款，理由也同款（到了落点只剩机器凭证，人分不出来了）；审查抓到过这个真漏洞：分流不产生 `node_relays` 行，只看中转线路的继承逻辑什么都看不见。另：路由规则指向不存在的 outbound 时 **`xray -test` 不报错**、流量静默直出，所以装配时自己查（`checkRuleTargets`）。详见 `docs/node-egress.md`。
