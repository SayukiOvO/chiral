...
## M5 — 用户端门户与在线记录 🚧 设计已定稿，实施中
面向端用户的第二个信任域：普通人自己注册登录，看自己有权限的线路、订阅链接与账户状态。
设计与逐条决策见 `docs/user-portal.md`。

- [x] 多管理员 + RBAC；审计日志（`c711d2c`）。
- [x] 节点离线告警（Telegram / webhook）（`eed7d78`）。
- [x] 登录 MFA：TOTP / Passkey / 邮箱验证码 / 恢复码（`f944e6b`、`fb375d4`）。
- [ ] 限流与登录加固——**开放注册的前置依赖**，同时修掉现存的 DoS 与用户名枚举面。
- [ ] 在线设备 / IP 记录（只记录，不告警；按用户可设展示阈值）。
- [ ] 门户后端：注册 / 登录 / 认领 / 找回，白名单投影，与管理端的鉴权边界。
- [ ] 门户前端：Vite 多入口，门户在 `/`、管理台移到 `/admin/`。

**关键实测结论**（Xray 26.3.27，见 `docs/user-portal.md` §9）：`online` 计的是**不同源 IP 数**而非会话数；`rmu` 与 `sib` 都**不能**掐断已建立的会话，只拒新连接——所以「超限自动断线」在内核层面不存在。
