# AGENTS.md — Chiral 项目约定

本文件是 Chiral 项目的权威上下文，供 Codex 在后续会话中快速对齐。人类协作者也应先读本文件。改动重大决策时，请同步更新本文件。
...
我想开发一个Xray的管理面板，这个项目可以说和Remnawave类似吧，首先呢，也是panel + core + agent的架构。必须支持docker和docker-compose部署。
在panel上，可以方便地添加节点，会给出docker-compose安装指令和鉴权token方便节点加入。节点加入后可以显示各种信息，比如CPU 内存之类的资源占用情况，网络速度，IP及其地区，xray内核版本，在xray上消耗的出入站流量等等。面板呢，应该注重美观，参考新款AWS和Gitlab的风格和美学设计，要支持日/夜间模式，支持跟随系统，且有移动端适配。
core和agent会管理每台主机上的Xray-core及其config.json。具体怎么管理呢，在panel上设置各种xray的preset json（这个是个web编辑器），或者说template，支持模板语法，即{{variable}}这种，这些模板语法或者变量呢，要单独搞个框，支持自定义值或通过某种算法生成的值（比如xray生成的一些密钥），这些template可以绑到一或多个节点上。随后呢，可以方便地管理每个节点的入站和出站（毕竟这俩都是数组嘛）。
要有用户管理的功能，即通过管理xray配置里的clients数组实现，不同的用户，他们能访问的入站也是不一样的，这点要做区分，而且还要有流量限制和过期日期限制（还有是否开启自动续期的选项）。要统计他们用了多少流量，还有随之而来的订阅链接，用户应该可以通过订阅链接获取专属于他们的配置文件，他们的配置文件里面的节点的UUID或者说密码什么的东西，要独一无二。还有，这个订阅链接应该支持根据用户的代理软件不同，给出的配置也不同，目前就适配clash(mihomo)，v2rayn（不确定用的是不是xray内核，或者说能不能用xray的config.json，用vless://这种链接可能会更好？），xray，stash（我不确定内核和clash是不是一样的）这几种客户端就可以了。
由于我写的xray配置比较复杂，xhttp上下行分离+后量子加密，基本没有订阅转换可以胜任，即客户端配置有无限种组合方式，我不希望有通过服务器入站策略转换成客户端配置导致的局限性，所以我打算客户端配置方面，也去做template，也要有模板语法，支持填充一些变量，比如服务器IP和端口（？）你要思考一下，怎么和上面的去做适配，做成一个自动化程度相对较高的东西。
