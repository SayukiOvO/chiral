...
import { api, type ExternalProxy, type ExternalSub, type Node } from "../api";
 ...
 * discovered.
 *
 * What the panel adds is the chain: an external node can be reached through one
 * of this fleet's own, so the provider sees the operator's node and not the
 * subscriber.
 */
      ...
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            {t("别人提供的订阅或单条链接。可为每个节点指定一台本机队节点作为前置，链式出站。")}
          </p>
        </div>
        ...
        <div className="mt-4 border-t border-line pt-4">
          {sub.proxies.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {sub.proxies.map((p) => (
                <ProxyRow key={p.id} subId={sub.id} proxy={p} nodes={nodes} onChanged={onChanged} />
              ))}
            </div>
          )}
        </div>
...
function ProxyRow({
  ...
  async function patch(p: { chain_node_id?: string; enabled?: boolean }) {
    setBusy(true);
    try {
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-x-3 gap-y-2", !proxy.enabled && "opacity-50")}>
      <button
      ...
      {/* The chain is the reason this page exists: the provider sees the
          operator's node rather than the subscriber. */}
      <select
        value={proxy.chain_node_id}
        disabled={busy}
        onChange={(e) => patch({ chain_node_id: e.target.value })}
